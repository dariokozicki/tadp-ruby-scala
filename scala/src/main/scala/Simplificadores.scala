...
package object Simplificadores {
  ...
  def simplificarColorAnidado(figura: Figura): Figura ={
    figura match {
      case ("color",(rgbs: List[Double], ("color", (rgbs2: List[Double], grupoColor2: Figura)))) =>
        if(rgbs.equals(rgbs2)) ("color", (rgbs, simplificarColorAnidado(grupoColor2))) else figura
      case (name: String, (coord: List[Double], fig: Figura)) => (name, (coord,(fig._1, simplificarColorAnidado(fig))))
      case _ => figura
    }
  }
